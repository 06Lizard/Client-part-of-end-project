...
std::future<int> Client::ListenForMessage()
{
    ...
    return std::async(std::launch::async, [this]() -> int
    {
        ...
        if (bytesReceived > 0)
        {
            ...
            /*
            Checking if the user has logged in by checking if the message contains things that hint at the user having successfully logged in. Not a secure or reliable solution, but I didn't have any other options
            because there is currently no other way of detecting a login from the server. Will work on a better solution if there is time.
            */
